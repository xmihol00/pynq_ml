...
int main()
{
        ...
        if (n > 1)
        {
            for (int i = 0; i < 1024; i+=4)
            {
                ...
                if (out.keep && !last)
                {
                    ...
                    if (output[output_idx-1] == 6066)
                    {
                        std::cout << "Found 6066 at " << output_idx-1 << std::endl;
                    }
                    output[output_idx++] = out.data.range(31, 16);
                    if (output[output_idx-1] == 11793)
                    {
                        std::cout << "Found 11793 at " << output_idx-1 << std::endl;
                    }
    ...
    std::cout << "Valid count: " << valid_count << " " << (valid_count == 256*255) << std::endl;
